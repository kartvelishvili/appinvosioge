...
const EnhancedMonthlyRevenueTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    const data = payload[0].payload;
    const { planned, paid, unpaid, overdue, invoices } = data;
      ...
      <div className="bg-white/95 backdrop-blur-md p-4 border border-slate-200 shadow-xl rounded-xl w-72 z-50">
        ...
        <div className="space-y-2 mb-4">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-mono font-bold text-slate-700">{planned?.toLocaleString() || 0} ₾</span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-slate-500 flex items-center gap-1.5">
            ...
            <span className="font-mono font-bold text-emerald-600">{paid?.toLocaleString() || 0} ₾</span>
          </div>
        </div>

        <div className="border-t border-slate-100 pt-3">
          <p className="text-[10px] uppercase font-bold text-slate-400 mb-2 tracking-wider">ინვოისების დაშლა ({invoices?.length || 0})</p>
...
export default EnhancedMonthlyRevenueTooltip;